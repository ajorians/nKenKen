...
void CreateSelector(struct Selector** ppSelector, struct SDL_Surface* pScreen, struct Config* pConfig, struct Metrics* pMetrics, KenKenLib kenken)
{
...
{
   struct Selector* pSelector = *ppSelector;
   pSelector->m_KenKen = NULL;//Does not own
...
void Move(struct Selector* pSelector, enum Direction eDirection)
{
   switch( eDirection ) {
      ...
      case Up:
         ...
         break;
      case Down:
         if( pSelector->m_nCurrentY < (GetKenKenHeight(pSelector->m_KenKen)-1) ) pSelector->m_nCurrentY++;
         break;
      case Left:
         if( pSelector->m_nCurrentX > 0 ) pSelector->m_nCurrentX--;
         break;
      case Right:
	 if( pSelector->m_nCurrentX < (GetKenKenWidth(pSelector->m_KenKen)-1) ) pSelector->m_nCurrentX++;
         break;
   }
}
